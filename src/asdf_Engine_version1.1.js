...
params=new EventParameter();this.mMouseOutEvents[this.mMouseOutEvents.length]=functionName;this.mMouseOutParams[this.mMouseOutParams.length]=params;}
function asdf_BackgroundStitcher(_id,_parent,positionX,positionY,bgColor,width,height,imgTop,imgMiddle,imgBottom,positionType,img_topH,img_topW,img_bottomH,img_bottomW,extra_css_class,initialShow,zIndex){if(_id==null){if(globals.debug>0)
...
this.mPosY=positionY;if(img_topH==null||img_topH==undefined){this.mImg_topH="0px";if(globals.debug>1)
alert("Warning: BackgroundStitcher: Image Top Height is not set - set to 0");}else{this.mImg_topH=img_topH;}
if(img_topW==null||img_topW==undefined){this.mImg_topW="0px";if(globals.debug>1)
alert("Warning: BackgroundStitcher: Image Top Width is not set - set to 0");}else{this.mImg_topW=img_topW;}
if(img_bottomH==null||img_bottomH==undefined){this.mImg_bottomH="0px";if(globals.debug>1)
alert("Warning: BackgroundStitcher: Image Bottom Height is not set - set to 0");}else{this.mImg_bottomH=img_bottomH;}
if(img_bottomW==null||img_bottomW==undefined){this.mImg_bottomW="0px";if(globals.debug>1)
alert("Warning: BackgroundStitcher: Image Bottom Width is not set - set to 0");}else{this.mImg_bottomW=img_bottomW;}
if(extra_css_class==null)
this.mExtraClassCSS="EXTRA_NOTSET";else
this.mExtraClassCSS=extra_css_class;if(bgColor==null)
this.mBgColor="transparent";else
...
asdf_BackgroundStitcher.prototype.show=function(){if(this.mDomTreeObject==null){this.mDomTreeObject=createDomObject(this,this.mId,"div",this.mType,this.mExtraClassCSS);this.mDomImageTop=createDomObjectDOM(this,this.mDomTreeObject,this.mId+"_imageTop","img",this.mType,this.mExtraClassCSS,this.mImageTop);this.mDomImageMiddle=createDomObjectDOM(this,this.mDomTreeObject,this.mId+"_imageMiddle","img",this.mType,this.mExtraClassCSS,this.mImageMiddle);this.mDomImageBottom=createDomObjectDOM(this,this.mDomTreeObject,this.mId+"_imageBottom","img",this.mType,this.mExtraClassCSS,this.mImageBottom);$(this.mDomTreeObject).mouseover(onMouseOver);$(this.mDomTreeObject).mouseout(onMouseOut);$(this.mDomTreeObject).click(onMouseClick);setObjectSize(this.mDomImageTop,this.mImg_topW,this.mImg_topH,"px","px");setObjectSize(this.mDomImageBottom,this.mImg_bottomW,this.mImg_bottomH,"px","px");this.mDomTreeObject.style.position=this.mPositionType;this.setPosition(this.mPosX,this.mPosY);this.setSize(this.mWidth,this.mHeight);this.mDomTreeObject.style.background=this.mBgColor;this.mDomTreeObject.style.zIndex=this.mZIndex;var topSize=new Size(this.mImg_topW,this.mImg_topH);var bottomSize=new Size(this.mImg_bottomW,this.mImg_bottomH);var middleSize=this.getImageSize(this.mDomImageMiddle);middleSize.y=getValueWithoutUnits(this.mHeight)-getValueWithoutUnits(topSize.y)-getValueWithoutUnits(bottomSize.y);if(middleSize.y<0){this.mDomImageBottom.style.height=getValueWithUnits(getValueWithoutUnits(bottomSize.y)+getValueWithoutUnits(middleSize.y));this.mDomImageMiddle.style.height="0px";}else{this.mDomImageMiddle.style.height=getValueWithUnits(middleSize.y);}
